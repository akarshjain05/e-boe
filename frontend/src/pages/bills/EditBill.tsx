...
export default function EditBill() {
  ...
  // Fetch real customers for dropdown
  ...
  useEffect(() => {
    if (bill) {
      form.reset({
        ...
        credit_period_months: bill.credit_period_months ?? undefined,
        currency_code: bill.currency_code,
        transaction_type: (bill as any).transaction_type || 'intra_state',
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => billService.updateBill(id!, data),
    onSuccess: () => {
  ...
  async function onSubmit(data: CreateBillValues) {
    ...
    // Auto populate drawee name if we have the customer record
    ...
    <div className="space-y-6 pb-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">Edit Bill</h1>
            <p className="text-zinc-500 dark:text-zinc-400 mt-1">Modify your draft bill of exchange.</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate(-1)}>Cancel</Button>
          <Button onClick={form.handleSubmit(onSubmit)} className="bg-indigo-600 hover:bg-indigo-700 shadow-sm gap-2" disabled={isSubmitting || isLoadingBill}>
