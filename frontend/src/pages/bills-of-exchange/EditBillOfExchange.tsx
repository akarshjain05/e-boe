...
export default function EditBillOfExchange() {
  const { id: boeId } = useParams();
  ...
  // Queries
  const { data: existingBoe, isLoading: isLoadingBoe } = useQuery({
    queryKey: ['bill-of-exchange', boeId],
    queryFn: () => boeService.getBillOfExchange(boeId!),
    enabled: !!boeId
  });
  ...
  // Pre-fill form when existingBoe loads
  useEffect(() => {
    if (existingBoe) {
      form.reset({
        customer_id: existingBoe.customer_id,
        issue_date: existingBoe.issue_date,
        due_date: existingBoe.due_date,
        place_of_issue: existingBoe.place_of_issue || "",
        description: existingBoe.description || "",
      });
      setSelectedInvoices(existingBoe.invoices?.map(i => i.bill_id) || []);
    }
  }, [existingBoe, form]);
  ...
  // Filter bills for the specific customer that are accepted/overdue and unpaid, plus currently selected ones
  const availableBills = useMemo(() => {
    return customerBills.filter(b => 
      b.customer_id === selectedCustomerId && 
      (
        (['accepted', 'overdue'].includes(b.status) && Number(b.outstanding_amount) > 0) ||
        selectedInvoices.includes(b.id)
      )
    );
  }, [customerBills, selectedCustomerId, selectedInvoices]);
  ...
  const updateMutation = useMutation({
    mutationFn: (data: Parameters<typeof boeService.updateBillOfExchange>[1]) => boeService.updateBillOfExchange(boeId!, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['bills-of-exchange'] });
      toast.success("Bill of Exchange updated successfully!");
      navigate('/bills-of-exchange');
    },
    onError: () => {
      toast.error("Failed to update Bill of Exchange.");
    }
  });
  ...
  if (isLoadingBoe) {
    return <div className="p-8 flex justify-center"><Loader2 className="animate-spin text-zinc-500 h-8 w-8" /></div>;
  }

  return (
    <div className="flex flex-col h-full bg-zinc-50 dark:bg-zinc-950">
      <div className="p-6 pb-2 border-b">
        <h1 className="text-2xl font-bold tracking-tight">Edit Bill of Exchange</h1>
      </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-4xl mx-auto">
            ...
            <Card className={!selectedCustomerId || selectedInvoices.length === 0 ? "opacity-50 pointer-events-none" : ""}>
              ...
              <CardFooter className="flex justify-end pt-6 border-t gap-4">
                <Button 
                  type="button" 
                  variant="outline"
                  onClick={() => navigate('/bills-of-exchange')}
                >
                  Cancel
                </Button>
                <Button 
                  ...
                  size="lg" 
                  className="gap-2" 
                  disabled={updateMutation.isPending || selectedInvoices.length === 0}
                >
